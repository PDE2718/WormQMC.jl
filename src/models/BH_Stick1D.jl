...
@kwdef struct BH_Stick1D <: BH_Parameters
...
N_sublatt(::Type{BH_Stick1D}) = 1
N_wldim(::Type{BH_Stick1D})::Int = 2 # can only be 2 or 3
N_nbs(::Type{BH_Stick1D})::Int = 4
N_hops(::Type{BH_Stick1D})::Int = 2

function get_nbs(H::BH_Stick1D, i::Integer)::NTuple{4,Int}
    return mod1.((i - 2, i - 1, i + 1, i + 2), H.Lx)
end
function get_hops(H::BH_Stick1D, i::Integer)::NTuple{2,Int}
    return mod1.((i - 1, i + 1), H.Lx)
end
function diagE(H::BH_Stick1D, i::Integer, ni::StateType, njs::NTuple{4,StateType})::f64
    return ni * (H.V * sum(njs) - H.μ)
end
function bond_weight(H::BH_Stick1D, i::Integer, j::Integer)::f64
    return H.J
end
function site_diff(H::BH_Stick1D, i::Integer, j::Integer)::CartesianIndex{4}
    dx = mod1(j - i + 1, H.Lx)
    return CartesianIndex(dx, 1, 1, 1)
end
function Wsheet(β::f64, H::BH_Stick1D)
    @assert H.Ly == 1
    Wsheet(β, zeros(StateType, H.Lx, H.Ly))
end

function simple_measure!(m::SimpleMeasure, x::Wsheet{2}, H::BH_Stick1D,
...
function simple_measure_names(::BH_Stick1D)
